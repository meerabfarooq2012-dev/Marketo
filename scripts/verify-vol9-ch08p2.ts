 ...
 * check whether the returned topic matches the expected math_vol9_ch08p2_* topic.
 */
import { readFileSync } from 'fs'

const data = JSON.parse(
  readFileSync('data/math-formulas-vol9-ch08p2.json', 'utf8')
) as {
  items: { question: string; topic: string; keywords: string[] }[]
}

const STORED_PREFIX = 'math_vol9_' // topicPrefix from route.ts VOLUME_CONFIG
const PART_PREFIX = 'math_vol9_ch08p2_' // for detecting cross-part hits
...
function craftQuery(q: { question: string; topic: string; keywords: string[] }): string {
  // Use the question as-is (already distinctive for ch08p2)
  return q.question
}
