/**
 * Verify ch08p4 production chat retrieval: 50 queries, target 100% direct hits.
 * Each query is the item's question. Expected returned topic = math_vol9_ch08p4_<topic>.
 */
...
async function main() {
  ...
  console.log(`Verifying ${file.items.length} ch08p4 items against production chat...\n`)
  ...
  for (let i = 0; i < file.items.length; i++) {
    ...
    const expectedTopic = STORED_PREFIX + item.topic // e.g. math_vol9_ch08p4_joint_pmf_discrete
    ...
    if (!result) {
      ...
      if (got === expectedTopic) {
        hit = 'direct'
      } else if (got.startsWith('math_vol9_ch08p4_')) {
        hit = 'cross-part'
