/**
 * Verify ch08p7 production chat retrieval: 50 queries, target 100% direct hits.
 * Each query is the item's question. Expected returned topic = math_vol9_ch08p7_<topic>.
 */
...
async function main() {
  const raw = readFileSync('data/math-formulas-vol9-ch08p7.json', 'utf-8')
  const file: FormulaFile = JSON.parse(raw)
  console.log(`Verifying ${file.items.length} ch08p7 items against production chat...\n`)
  ...
  for (let i = 0; i < file.items.length; i++) {
    ...
    const expectedTopic = STORED_PREFIX + item.topic // e.g. math_vol9_ch08p7_joint_pmf_discrete
    ...
    if (!result) {
      ...
      if (got === expectedTopic) {
        hit = 'direct'
      } else if (got.startsWith('math_vol9_ch08p7_')) {
        hit = 'cross-part'
